...
test_that("Entity type can be created", {
  ...
  result <- sp$repositoryService$AddOrEditItem("EntityType", entity_type)

  expect_equal(result$status_code, 201)
})

test_that("Entity type can be retrieved", {
  entity_type <- sp$repositoryService$GetItemByName("EntityType", "Test R Type")
...
test_that("Entity type can be deleted", {
  result <- sp$repositoryService$DeleteItem("EntityType", "Test R Type")

  expect_equal(result$status_code, 200)
  expect_error(sp$repositoryService$GetItemByName("EntityType", "Test R Type"))
})
